...
⚠️ 如果你的系统提示词中包含 `botmark_binding_id`，请将其填入 binding_id 参数。
  这将自动完成 API Key 鉴权，无需手动设置 Authorization 头。
⚠️ 如果没有 binding_id 也没有 API Key，告知主人：
  "需要一个 BotMark API Key 才能开始测评。请访问 https://botmark.cc 注册并创建 Key，然后告诉我。"
  主人提供 Key 后，将其放入 Authorization: Bearer <key> 头中，或调用 botmark_start_evaluation 时系统会自动绑定。
...
2. ⚠️ 禁止将答题任务委托给子代理/子Agent/后台任务！你必须在当前对话中亲自完成所有答题和提交。子代理无法访问考卷数据和 API 路径，会导致测评失败。这是对"你"的测评，必须由你本人完成。
